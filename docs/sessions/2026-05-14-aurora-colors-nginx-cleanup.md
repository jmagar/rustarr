...
12. SSHed into edgehost; read `mcp-server.conf`, `mcp-location.conf`, and all 7 existing MCP proxy confs
...
15. Ran `tailscale status` to identify devices: devhost=198.51.100.1, edgehost=198.51.100.3, nashost=198.51.100.2
16. Rewrote/fixed all 7 configs + created 2 new confs; deleted old `syslog-mcp.subdomain.conf`
17. `nginx -t` passed; `nginx -s reload` successful on edgehost
...
- All configs on edgehost had `mcp_upstream_port` values that didn't match actual running containers — the real ports are in the 40xxx range (40010–40060), not the old values (6970, 8001, 9158).
...
- **syslog-mcp → syslog rename**: The domain was already `syslog.example.internal`; the filename `syslog-mcp.subdomain.conf` was inconsistent. Renamed to `syslog.subdomain.conf`.
- **apprise MCP upstream on devhost, not nashost**: The apprise web service runs on nashost (198.51.100.2:8766), but apprise-mcp (the Rust sidecar) runs on devhost (198.51.100.1:40050). Split upstream/mcp_upstream accordingly.
...
### NGINX proxy configs on edgehost (`/mnt/appdata/swag/nginx/proxy-confs/`)
...
- **Modified** `apprise.subdomain.conf` — complete rewrite from 2023 old format to standardized pattern; added MCP sections; `mcp_upstream` = `198.51.100.1:40050`
...
- **Created** `tailscale.subdomain.conf` — `ts.example.internal`, `198.51.100.1:40040`, authelia on `/`
- **Created** `rmcp-rustarr.subdomain.conf` — `rmcp.example.internal`, `198.51.100.1:40060`, authelia on `/`
...
# NGINX audit + fix
ssh edgehost "cat /mnt/appdata/swag/nginx/mcp-server.conf"
ssh edgehost "cat /mnt/appdata/swag/nginx/proxy-confs/[all confs]"
docker ps --format '{{.Names}}\t{{.Ports}}' | sort   # actual port mapping
tailscale status                                        # device → IP mapping
ssh edgehost "docker exec swag nginx -t && nginx -s reload"  # validated clean
```
...
| lab.example.internal | Authelia gate on `/` (double-auth with built-in OAuth) | No Authelia — passes directly to lab MCP server |
| syslog.example.internal | No auth on `/`; config named `syslog-mcp.subdomain.conf` | Authelia on `/`; config named `syslog.subdomain.conf` |
| axon.example.internal | No auth on `/`; upstream port 49010 (wrong); redundant OAuth location blocks | Authelia on `/`; upstream port 8001; clean standard structure |
| gotify MCP endpoint | Routed to port 9158 (not running) | Routed to port 40020 (gotify-mcp actual port) |
| apprise proxy | 2023-era old format; no MCP sections | Standardized format; MCP routed to `198.51.100.1:40050` |
...
| ts.example.internal | Did not exist | New conf; Tailscale MCP at `198.51.100.1:40040`; authelia on `/` |
| rmcp.example.internal | Did not exist | New conf; rmcp-rustarr at `198.51.100.1:40060`; authelia on `/` |
...
| `nginx -t` on edgehost | syntax ok | syntax ok | ✓ |
| `nginx -s reload` on edgehost | reloaded | reloaded | ✓ |
...
- **apprise-mcp port**: The apprise-mcp sidecar (`198.51.100.1:40050`) must be running for MCP calls to work. The upstream Apprise web UI (`198.51.100.2:8766`) is unaffected.
- **syslog-mcp.subdomain.conf deleted**: The old file is gone. The new `syslog.subdomain.conf` is the replacement. No DNS change was needed (domain was already `syslog.example.internal`).

## Open Questions

- **unraid upstream**: The `upstream` and `mcp_upstream` for unraid both point to unrust (`198.51.100.1:40010`). If the user wants `/` to show the actual unRAID management UI (not the unrust status page), the upstream needs to point to the unRAID server's LAN IP instead.
- **unifi upstream**: Same question — does `unifi.example.internal` `/` should show rustifi's status page or the actual UniFi controller UI?
- **axon `ansi_bold` unused**: Added `ansi_bold()` helper to axon_rust logging but it is not currently called. Remove or use it.
- **apprise-mcp deployment**: apprise-mcp runs locally on devhost. If it restarts or the port changes, the NGINX conf needs updating.

## Next Steps

- Deploy syslog-mcp with the new aurora logging (rebuild container on devhost)
- Deploy axon_rust with updated logging (rebuild container on devhost)
- Verify `ts.example.internal` and `rmcp.example.internal` resolve correctly once DNS/cert propagates
